...
N_samples = 5
M = 200 #Number of points to classify
b = torch.randn(N_samples, M)
#choose parameter for generating the points in the cluster
A = 1/torch.sqrt(torch.tensor(P)) * torch.randn(N_samples, M, P)


f = objectives.logsumexp
xstar = None
eigvals = torch.sort(torch.real(torch.linalg.eigvals((A.transpose(-1,-2)@A))), dim=-1)[0]
...
ax.set_title(r'LogSumExp on synthetic dataset')
...
fig.tight_layout()
fig.savefig('logsumexp_synthetic_comparison.pdf')
fig.show()
# %%
