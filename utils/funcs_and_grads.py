...
class function_to_minimize:
    ...
    def evalHessf(self, x):
        ...
        if x.shape[0]==1 and len(x.shape)>1: #if only one sample
            ...
            #warning: not optimized at all, overkill but works
...
def logistic(X, A, b):
    ...
    loglike = torch.sum(torch.log(1. + torch.exp(-b * correl)), dim=-1) #vector of all the log likelihood b is binary \in {-1,1}
...
def logsumexp(X, A, b):
    ...
    if len(X.shape) > len(b.shape):
        b = b[None]
    correl = (A @ X.unsqueeze(-1)).squeeze()  #Linear model times x
    loss = torch.logsumexp(correl - b, dim=-1) #numerically stabilized pytorch version
    Y = X.clone()  #Used to avoid problems with backpropagating at multiple iterations
    regul = 1 / (2.*b.shape[-1]) * torch.sum(Y**2, dim=-1) #Small L2 regul
    g = loss + regul #returns one value per data sample
    return g
