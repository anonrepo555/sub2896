...
class LOA_BFGS_Model(nn.Module):
    ...
    def reset_mat(self, n_traj, factor=1, first_guess=None):  #initialize matrices to predict with identity
        if n_traj == 1:
            self.predicted_mat = self.diag(factor*torch.ones(self.P)) if first_guess is None else first_guess.clone()
    ...
    def forward(self, grad, gradm1, dm1, previous_gamma=None):
        previous_gamma = self.gamma if previous_gamma is None else previous_gamma
        ...
        x = self.construct_features(grad, gradm1, dm1, previous_gamma=previous_gamma) #Construct the features Ik
        # We need a tensor of size batchsize*P*n_features (to operate along the feature dim)
        ...
        ##FF layers loop
        fullskip = self.fullskip_layer(x).squeeze(-1) if self.use_fullskipco is True else torch.zeros_like(x).squeeze(-1)
