...
def ridge(X, A, b, lmbda=1e-1):
    ...
    return g + lmbda/2. * torch.sum(X, dim=-1)
    ...
    if Wolfe_LS:
        ...
        with torch.no_grad():
            ...
            while n_LS < 5 and torch.sum(Wolfecrit)>0:  #Try 5 times the line-search
                ...
                Wolfecrit = (temp_values - current_value - c1 * stepsize.squeeze() * stepXgrad) > 0   #this criterion must be negative to exit the LS loop
...
list_values_HB = algos.HeavyBall(x0, func, gamma=1.99*gamma_1ststep, alpha=2.*torch.sqrt(eigvals[0]), niter_max=niter_max)
#%%
########### Plot the result ############
def plot_area(xarray, values, ax, label, color=None, zorder=5): 
    below, _ = torch.min(values, dim=0) 
    above, _ = torch.max(values, dim=0)
    ax.plot(xarray, torch.median(values, dim=0)[0], lw=3, label=label,color=color, zorder=zorder)
...
ax.set_title(r'Ridge regression on California housing dataset, $\lambda=100$')
...
fig.tight_layout()
fig.savefig('ridge_california_comparison.pdf')
fig.show()
# %%
